...
describe('Basic Auth Middleware', () => {
    test('should reject request without authorization header', async () => {
        ...
        await registerBasicAuthMiddleware(fastify, { username: 'test', password: 'test123' });
    ...
    test('should accept request with valid credentials', async () => {
        ...
        const response = await fastify.inject({
            ...
            url: '/test',
            headers: {
                authorization: 'Basic ' + Buffer.from('test:test123').toString('base64')
            }
        });
    ...
    test('should attach user info to request when authenticated', async () => {
        ...
        await registerBasicAuthMiddleware(fastify, { username: 'testuser', password: 'testpass' });
        const response = await fastify.inject({
            method: 'GET',
            url: '/test',
            headers: {
