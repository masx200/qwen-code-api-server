...
export async function parseArguments(argv) {
    const yargsInstance = yargs(/* hideBin */ argv /* process.argv */)
        .scriptName("qwen")
        .usage("$0 [options]", "Qwen Code - Launch an interactive CLI, use -p/--prompt for non-interactive mode")
        .option("model", {
        alias: "m",
        type: "string",
        description: `Model`,
        ...
        type: "boolean",
        description: "If true, when refreshing memory, QWEN.md files should be loaded from all directories that are added. If false, QWEN.md files should only be loaded from the primary working directory.",
        default: false,
    })
